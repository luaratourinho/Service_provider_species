...
model <- pa ~bio_10+bio_14+bio_18+bio_2+bio_3
...
file = "./ENM/data/03_clean_df_thin.csv"
...
raster_files <-
  list.files(paste0("./ENM/outputs/", sp.n, "/Pres_env_crop"),
             full.names = T,
             'tif$|bil$')
#head(raster_files)
predictors <- stack(raster_files)

# Read your future environmental rasters 

raster_files2 <-
  list.files(paste0("./ENM/outputs/", sp.n, "/Fut_env_crop"),
             full.names = T,
             'tif$|bil$')
head(raster_files2)
...
target_dir = paste(paste0("./ENM/outputs/", sp.n, "/results/", sep=""))
...
if(all(TSS < tss.lim)){
  ...
  save.image("./ENM/outputs/my_analysis.rData")
...
# Use these futures if you are considering other ssp or GCM
# future_variable2.bc <- list()
# future_variable2.bc.bin <- list()
# future_variable3.bc <- list()
# future_variable3.bc.bin <- list()
...
for(i in 1:k){
  ...
  if(bcTSS[[i]] >= tss.lim){
    ...
    # future_variable3.bc[[i]] <- predict(future_variable3, bc[[i]])
    # future_variable3.bc.bin[[i]] <- future_variable3.bc[[i]] > bcthres[[i]]
    ...
    # future_variable3.bc[[i]] <- NULL
    # future_variable3.bc.bin[[i]] <- NULL
  }
}
...
# future_variable3.bc <- Filter(Negate(is.null), future_variable3.bc)
# future_variable3.bc.bin <- Filter(Negate(is.null), future_variable3.bc.bin)
...
# future_variable3.bc.ens <- Reduce('+', future_variable3.bc.bin)
# tval <- unique(future_variable3.bc.ens)
# tval <- tval[tval != 0]
# tval <- median(tval)
# future_variable3.bc.ens.bin <- future_variable3.bc.ens >= tval
...
if(length(cur.bc) != 0) {
  ...
  # for(z in 1:length(future_variable3.bc)){
  #   adeq = future_variable3.bc[[z]]
  #   minimo <- min(adeq[], na.rm=T)
  #   maximo <- max(adeq[], na.rm=T)
  #   adeq_norm <- function(x) {(x-minimo)/(maximo-minimo)}
  #   future_variable3.bc[[z]] <- calc(adeq, adeq_norm)
  # }
  ...
  # x <- stack(future_variable3.bc)
  # future_variable3.bc.cont <- calc(x, fun = mean)
  
} else {
  cur.bc.cont <- NULL
  future_variable.bc.cont <- NULL
  # future_variable2.bc.cont <- NULL
  # future_variable3.bc.cont <- NULL
}
...
# future_variable3.gm <- list()
# future_variable3.gm.bin <- list()
...
for(i in 1:k){
  ...
  if(gmTSS[[i]] >= tss.lim){
    ...
    # future_variable3.gm[[i]] <- predict(future_variable3, gm[[i]])
    # future_variable3.gm.bin[[i]] <- future_variable3.gm[[i]] > gmthres[[i]]
    ...
    # future_variable3.gm[[i]] <- NULL
    # future_variable3.gm.bin[[i]] <- NULL
  }
}
...
# future_variable3.gm <- Filter(Negate(is.null), future_variable3.gm)
# future_variable3.gm.bin <- Filter(Negate(is.null), future_variable3.gm.bin)
...
# future_variable3.gm.ens <- Reduce('+', future_variable3.gm.bin)
# tval <- unique(future_variable3.gm.ens)
# tval <- tval[tval != 0]
# tval <- median(tval)
# future_variable3.gm.ens.bin <- future_variable3.gm.ens >= tval
...
for(z in 1:length(future_variable.gm)){
  ...
  # for(z in 1:length(future_variable3.gm)){
  #   adeq = future_variable3.gm[[z]]
  #   minimo <- min(adeq[], na.rm=T)
  #   maximo <- max(adeq[], na.rm=T)
  #   adeq_norm <- function(x) {(x-minimo)/(maximo-minimo)}
  #   future_variable3.gm[[z]] <- calc(adeq, adeq_norm)
  # }
...
# x <- stack(future_variable3.gm)
# future_variable3.gm.cont <- calc(x, fun = mean)

} else {
  cur.gm.cont <- NULL
  future_variable.gm.cont <- NULL
  # future_variable2.gm.cont <- NULL
  # future_variable3.gm.cont <- NULL
}
...
# future_variable3.rf <- list()
# future_variable3.rf.bin <- list()
...
for(i in 1:k){
  ...
  if(rfTSS[[i]] >= tss.lim){
    ...
    # 
    # future_variable3.rf[[i]] <- predict(future_variable3, rf[[i]])
    # future_variable3.rf.bin[[i]] <- future_variable3.rf[[i]] > rfthres[[i]]
    ...
    # future_variable3.rf[[i]] <- NULL
    # future_variable3.rf.bin[[i]] <- NULL
...
# future_variable3.rf <- Filter(Negate(is.null), future_variable3.rf)
# future_variable3.rf.bin <- Filter(Negate(is.null), future_variable3.rf.bin)
...
# future_variable3.rf.ens <- Reduce('+', future_variable3.rf.bin)
# tval <- unique(future_variable3.rf.ens)
# tval <- tval[tval != 0]
# tval <- median(tval)
# future_variable3.rf.ens.bin <- future_variable3.rf.ens >= tval
...
# }
#   
#   for(z in 1:length(future_variable3.rf)){
#     adeq = future_variable3.rf[[z]]
#     if(sum(adeq[], na.rm=T)!=0){
#       minimo <- min(adeq[], na.rm=T)
#       maximo <- max(adeq[], na.rm=T)
#       adeq_norm <- function(x) {(x-minimo)/(maximo-minimo)}
#       future_variable3.rf[[z]] <- calc(adeq, adeq_norm)
#     }
#   }
...
# 
# x <- stack(future_variable3.rf)
# future_variable3.rf.cont <- calc(x, fun = mean)

} else {
  cur.rf.cont <- NULL
  future_variable.rf.cont <- NULL
  # future_variable2.rf.cont <- NULL
  # future_variable3.rf.cont <- NULL
}
...
# future_variable3.mx <- list()
# future_variable3.mx.bin <- list()
...
for(i in 1:k){
  ...
  if(mxTSS[[i]] >= tss.lim){
    ...
    # 
    # future_variable3.mx[[i]] <- predict(future_variable3, mx[[i]])
    # future_variable3.mx.bin[[i]] <- future_variable3.mx[[i]] > mxthres[[i]]
    ...
    # future_variable3.mx[[i]] <- NULL
    # future_variable3.mx.bin[[i]] <- NULL
  }
}
...
# future_variable3.mx <- Filter(Negate(is.null), future_variable3.mx)
# future_variable3.mx.bin <- Filter(Negate(is.null), future_variable3.mx.bin)
...
#   }
# 
# future_variable3.mx.ens <- Reduce('+', future_variable3.mx.bin)
# tval <- unique(future_variable3.mx.ens)
# tval <- tval[tval != 0]
# tval <- median(tval)
# future_variable3.mx.ens.bin <- future_variable3.mx.ens >= tval
# 
# if(!is.null(future_variable3.mx.ens)){
#   future_variable3.mx.ens.bin <- future_variable3.mx.ens >= tval} else {
#     future_variable3.mx.ens.bin <- NULL
#   }
...
# }
#   
#   for(z in 1:length(future_variable3.mx)){
#     adeq = future_variable3.mx[[z]]
#     if(sum(adeq[], na.rm=T)!=0){
#       minimo <- min(adeq[], na.rm=T)
#       maximo <- max(adeq[], na.rm=T)
#       adeq_norm <- function(x) {(x-minimo)/(maximo-minimo)}
#       future_variable3.mx[[z]] <- calc(adeq, adeq_norm)
#     }
#   }
...
# 
# x <- stack(future_variable3.mx)
# future_variable3.mx.cont <- calc(x, fun = mean)

} else {
  cur.mx.cont <- NULL
  future_variable.mx.cont <- NULL
  # future_variable2.mx.cont <- NULL
  # future_variable3.mx.cont <- NULL
}
...
# future_variable3.sv <- list()
# future_variable3.sv.bin <- list()
...
for(i in 1:k){
  ...
  if(svTSS[[i]] >= tss.lim){
    ...
    # 
    # future_variable3.sv[[i]] <- predict(future_variable3, sv[[i]])
    # future_variable3.sv.bin[[i]] <- future_variable3.sv[[i]] > svthres[[i]]
    ...
    # future_variable3.sv[[i]] <- NULL
    # future_variable3.sv.bin[[i]] <- NULL
  }
}
...
# future_variable3.sv <- Filter(Negate(is.null), future_variable3.sv)
# future_variable3.sv.bin <- Filter(Negate(is.null), future_variable3.sv.bin)
...
if(!is.null(future_variable.sv.ens)){
  future_variable.sv.ens.bin <- future_variable.sv.ens >= tval} else {
    future_variable.sv.ens.bin <- NULL
  }
...
# 
# if(!is.null(future_variable2.sv.ens)){
#   future_variable2.sv.ens.bin <- future_variable2.sv.ens >= tval} else {
#     future_variable2.sv.ens.bin <- NULL
#   }
# 
# future_variable3.sv.ens <- Reduce('+', future_variable3.sv.bin)
# tval <- unique(future_variable3.sv.ens)
# tval <- tval[tval != 0]
# tval <- median(tval)
# future_variable3.sv.ens.bin <- future_variable3.sv.ens >= tval
# 
# if(!is.null(future_variable3.sv.ens)){
#   future_variable3.sv.ens.bin <- future_variable3.sv.ens >= tval} else {
#     future_variable3.sv.ens.bin <- NULL
#   }
...
# }
#   
#   for(z in 1:length(future_variable3.sv)){
#     adeq = future_variable3.sv[[z]]
#     minimo <- min(adeq[], na.rm=T)
#     maximo <- max(adeq[], na.rm=T)
#     adeq_norm <- function(x) {(x-minimo)/(maximo-minimo)}
#     future_variable3.sv[[z]] <- calc(adeq, adeq_norm)
#   }
...
# 
# x <- stack(future_variable3.sv)
# future_variable3.sv.cont <- calc(x, fun = mean)

} else {
  cur.sv.cont <- NULL
  future_variable.sv.cont <- NULL
  # future_variable2.sv.cont <- NULL
  # future_variable3.sv.cont <- NULL
}
...
algo.future_variable <- c(#future_variable.bc.ens.bin, 
                          future_variable.gm.ens.bin, 
                          future_variable.rf.ens.bin, 
                          future_variable.mx.ens.bin, 
                          future_variable.sv.ens.bin)

#ignores NULL rasters but not NA
algo.future_variable <-
  algo.future_variable[lapply(algo.future_variable, length) > 0]

ens.future_variable <- Reduce('+', algo.future_variable)
tval <- unique(ens.future_variable)
tval <- tval[tval != 0]
tval <- median(tval)
ens.future_variable.bin <- ens.future_variable >= tval # this one! ensemble for future binary model


# For continuous model

if (length(future_variable.bc) != 0){
...
if (length(future_variable.gm) != 0){
...
if (length(future_variable.rf) != 0){
...
if (length(future_variable.mx) != 0){
...
if (length(future_variable.sv) != 0){
...
stack_fut <- c(st1, st2, st3, st4, st5)
st_fut <- stack(stack_fut)
w.sem.os.nulos <- w[w >= tss.lim]
ens2.future_variable <- weighted.mean(st_fut, w.sem.os.nulos) # this one! ensemble for future continuous model
ens.fut.cont <- ens2.future_variable
ens2.fut.sd.w <- sum(w.sem.os.nulos * (st_fut - ens.fut.cont)^2)
ens2.fut.sd.w <- sqrt(ens2.fut.sd.w) # uncertainty
...
if(length(wgm.sem.os.nulos) == 0){
  print('glm is null')
  future_variable.gm.mean.w <- NULL
...
# ens2.fut.sd.w2 <- sum(w.sem.os.nulos * (st_fut2 - ens.fut.cont2)^2)
...
# if(length(wgm.sem.os.nulos) == 0){
#   print('glm is null')
#   future_variable2.gm.mean.w <- NULL
...
# 
# # Ensemble (future) GCM3 -------------------------------------------------------------
# 
# # For binary model
# 
# # binarios
# algo.future_variable3 <- c(future_variable3.bc.ens.bin, future_variable3.gm.ens.bin, future_variable3.rf.ens.bin, future_variable3.mx.ens.bin, future_variable3.sv.ens.bin)
# algo.future_variable3 <- algo.future_variable3[lapply(algo.future_variable3, length) > 0] ##ignores NULL rasters but not NA
# #algo.future_variable2 <- list()
# 
# ## Ignores rasters that are all NA
# #for (i in 1:length(algo.future_variable)){
...
# #}
# 
# ens.future_variable3 <- Reduce('+', algo.future_variable3)
# tval <- unique(ens.future_variable3)
# tval <- tval[tval != 0]
# tval <- median(tval)
# ens.future_variable3.bin <- ens.future_variable3 >= tval
# 
# 
# 
# if (length(future_variable3.bc) != 0){
#   st1 <- stack(future_variable3.bc)
#   for(i in 1:length(future_variable3.bc)){
#     min_max <- range(future_variable3.bc[[i]][], na.rm=T)
#     #write.table(min_max, paste("future_variable.bc", i, sp.n, ".txt",  sep="_"))
#   }} else {
#     st1 <- NULL
#   }
# 
# 
# # continuous with ifelse
# 
# if (length(future_variable3.gm) != 0){
#   st2 <- stack(future_variable3.gm)
#   for(i in 1:length(future_variable3.gm)){
#     min_max <- range(future_variable3.gm[[i]][], na.rm=T)
#     #write.table(min_max, paste("future_variable.gm", i, sp.n, ".txt",  sep="_"))
#   }} else{
#     st2 <- NULL
#   }
# 
# 
# if (length(future_variable3.rf) != 0){
#   st3 <- stack(future_variable3.rf)
#   for(i in 1:length(future_variable3.rf)){
#     min_max <- range(future_variable3.rf[[i]][], na.rm=T)
#     #write.table(min_max, paste("future_variable.rf", i, sp.n, ".txt",  sep="_"))
#   }} else {
#     st3 <- NULL
#   }
# 
# 
# if (length(future_variable3.mx) != 0){
#   st4 <- stack(future_variable3.mx)
#   for(i in 1:length(future_variable3.mx)){
#     min_max <- range(future_variable3.mx[[i]][], na.rm=T)
#     #write.table(min_max, paste("future_variable.mx", i, sp.n, ".txt",  sep="_"))
#   }} else {
#     st4 <- NULL
#   }
# 
# 
# if (length(future_variable3.sv) != 0){
#   st5 <- stack(future_variable3.sv)
#   for(i in 1:length(future_variable3.sv)){
#     min_max <- range(future_variable3.sv[[i]][], na.rm=T)
#     #write.table(min_max, paste("future_variable.sv", i, sp.n, ".txt",  sep="_"))
#   }} else {
#     st5 <- NULL
#   }
# 
# 
# stack_fut3 <- c(st1, st2, st3, st4, st5)
# st_fut3 <- stack(stack_fut3)
# w.sem.os.nulos <- w[w >= tss.lim]
# ens2.future_variable3 <- weighted.mean(st_fut3, w.sem.os.nulos)
# ens.fut.cont3 <- ens2.future_variable3
# ens2.fut.sd.w3 <- sum(w.sem.os.nulos * (st_fut3 - ens.fut.cont3)^2)
# ens2.fut.sd.w3 <- sqrt(ens2.fut.sd.w3)
# 
# 
# # Uncertainty future ------------------------------------------------------
# 
...
# if(length(wbc.sem.os.nulos) == 0){
#   print('bioclim is null')
#   future_variable3.bc.mean.w <- NULL
#   future_variable3.bc.sd.w <- NULL
# } else {
#   future_variable3.bc.mean.w <- weighted.mean(st1, wbc.sem.os.nulos)
#   future_variable3.bc.sd.w <- sum(wbc.sem.os.nulos * (st1 - future_variable3.bc.mean.w)^2)
#   future_variable3.bc.sd.w <- sqrt(future_variable3.bc.sd.w) }
# 
# wgm <- c(gmTSSval)
# wgm.sem.os.nulos <- wgm[wgm >= tss.lim]
# if(length(wgm.sem.os.nulos) == 0){
#   print('glm is null')
#   future_variable3.gm.mean.w <- NULL
#   future_variable3.gm.sd.w <- NULL
# } else {
#   future_variable3.gm.mean.w <- weighted.mean(st2, wgm.sem.os.nulos)
#   future_variable3.gm.sd.w <- sum(wgm.sem.os.nulos * (st2 - future_variable3.gm.mean.w)^2)
#   future_variable3.gm.sd.w <- sqrt(future_variable3.gm.sd.w)}
# 
# wrf <- c(rfTSSval)
# wrf.sem.os.nulos <- wrf[wrf >= tss.lim]
# if(length(wrf.sem.os.nulos) == 0){
#   print('rf is null')
#   future_variable3.rf.mean.w <- NULL
#   future_variable3.rf.sd.w <- NULL
# } else {
#   future_variable3.rf.mean.w <- weighted.mean(st3, wrf.sem.os.nulos)
#   future_variable3.rf.sd.w <- sum(wrf.sem.os.nulos * (st3 - future_variable3.rf.mean.w)^2)
#   future_variable3.rf.sd.w <- sqrt(future_variable3.rf.sd.w)}
# 
# wmx <- c(mxTSSval)
# wmx.sem.os.nulos <- wmx[wmx >= tss.lim]
# if(length(wmx.sem.os.nulos) == 0){
#   print('mx is null')
#   future_variable3.mx.mean.w <- NULL
#   future_variable3.mx.sd.w <- NULL
# } else {
#   future_variable3.mx.mean.w <- weighted.mean(st4, wmx.sem.os.nulos)
#   future_variable3.mx.sd.w <- sum(wmx.sem.os.nulos * (st4 - future_variable3.mx.mean.w)^2)
#   future_variable3.mx.sd.w <- sqrt(future_variable3.mx.sd.w)}
# 
# wsv <- c(svTSSval)
# wsv.sem.os.nulos <- wsv[wsv >= tss.lim]
# if(length(wsv.sem.os.nulos) == 0){
#   print('svm is null')
#   future_variable3.sv.mean.w <- NULL
#   future_variable3.sv.sd.w <- NULL
# } else {
#   future_variable3.sv.mean.w <- weighted.mean(st5, wsv.sem.os.nulos)
#   future_variable3.sv.sd.w <- sum(wsv.sem.os.nulos * (st5 - future_variable3.sv.mean.w)^2)
#   future_variable3.sv.sd.w <- sqrt(future_variable3.sv.sd.w)}
# 
# ####
# 
# ens.fut3 <- ens.future_variable3.bin
# tval <- unique(ens.fut3)
# tval <- tval[tval != 0]
# tval <- median(tval)
# ens.fut.bin3 <- ens.fut3 >= tval
# 
# 
# # Ensemble GCMs
# 
# #Binary
# ens_gcm_bin <- c(ens.future_variable.bin + ens.future_variable2.bin + ens.future_variable3.bin)
# ens_gcm_bin <- ens_gcm_bin[lapply(ens_gcm_bin, length) > 0]
# 
# ens_gcm_all_bin <- Reduce('+', ens_gcm_bin)
# tval <- unique(ens_gcm_all_bin)
# tval <- tval[tval != 0]
# tval <- median(tval)
# ens_gcm_all_bin <- ens_gcm_all_bin >= tval
# 
# #Continuous
# ens_gcm_cont <- (ens.fut.cont + ens.fut.cont2 + ens.fut.cont3)/3
...
if(!is.null(ens.cur)){
  ...
  writeRaster(ens2.future_variable,
    file = paste(target_dir, '/Fut_bcc.cont_', sp.n, '.asc', sep = ""),
    overwrite = TRUE)
  # writeRaster(ens2.future_variable2, file = paste(target_dir, '/Fut_ipsl.cont_', sp.n, '.asc', sep=""),overwrite=TRUE)
  # writeRaster(ens2.future_variable3, file = paste(target_dir, '/Fut_miroc.cont_', sp.n, '.asc', sep=""),overwrite=TRUE)
  # writeRaster(ens_gcm_cont, file = paste(target_dir, '/Fut_all.cont_', sp.n, '.asc', sep=""),overwrite=TRUE)
...
if(!is.null(ens.cur.bin)){
  ...
  writeRaster(ens.future_variable.bin,
    file = paste(target_dir, '/Fut_bcc.bin_', sp.n, '.asc', sep = ""),
    overwrite = TRUE)
  
# writeRaster(ens.future_variable2.bin, file = paste(target_dir, '/Fut_ipsl.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
# writeRaster(ens.future_variable3.bin, file = paste(target_dir, '/Fut_miroc.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
# writeRaster(ens_gcm_all_bin, file = paste(target_dir, '/Fut_all.bin_', sp.n, '.asc', sep=""),overwrite=TRUE)
...
# writeRaster(ens2.fut.sd.w,file = paste(target_dir, 'ens.Fut_bcc.', BCC, '.sd.w_', sp.n, '.asc', sep = overwrite = TRUE)
# writeRaster(ens2.fut.sd.w2, file = paste(target_dir, 'ens.Fut_ipsl.', IPSL, '.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# writeRaster(ens2.fut.sd.w3, file = paste(target_dir, 'ens.Fut_miroc.', MIROC, '.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
...
if(!is.null(future_variable.gm.sd.w)) {
  writeRaster(future_variable.gm.sd.w,
    file = paste(target_dir, 'Fut_', '.gm.sd.w_', sp.n, '.asc', sep = ""),
    overwrite = TRUE)
}

# if(!is.null(future_variable2.gm.sd.w)){
#   writeRaster(future_variable2.gm.sd.w, file = paste(target_dir, 'Fut_', IPSL, '.gm.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
#   #writeRaster(future_variable.rf.mean.w, file = paste(target_dir, '/mpi8570.rf.mean.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# 
# if(!is.null(future_variable3.gm.sd.w)){
#   writeRaster(future_variable3.gm.sd.w, file = paste(target_dir, 'Fut_', MIROC, '.gm.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }


if(!is.null(future_variable.rf.sd.w)){
writeRaster(future_variable.rf.sd.w, 
            file = paste(target_dir, '/Fut_', '.rf.sd.w_', sp.n, '.asc', sep=""),
            overwrite=TRUE)
}

# if(!is.null(future_variable2.rf.sd.w)){
#   writeRaster(future_variable2.rf.sd.w, file = paste(target_dir, '/Fut_', IPSL, '.rf.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
# 
# if(!is.null(future_variable3.rf.sd.w)){
#   writeRaster(future_variable3.rf.sd.w, file = paste(target_dir, '/Fut_', MIROC, '.rf.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
...
writeRaster(future_variable.mx.sd.w, 
            file = paste(target_dir, '/Fut_', '.mx.sd.w_', sp.n, '.asc', sep=""),
            overwrite=TRUE)
}

# if(!is.null(future_variable2.mx.sd.w)){
#   writeRaster(future_variable2.mx.sd.w, file = paste(target_dir, '/Fut_', IPSL, '.mx.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
# 
# if(!is.null(future_variable3.mx.sd.w)){
#   writeRaster(future_variable3.mx.sd.w, file = paste(target_dir, '/Fut_', MIROC, '.mx.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
...
writeRaster(future_variable.sv.sd.w, 
            file = paste(target_dir, '/Fut_', '.sv.sd.w_', sp.n, '.asc', sep=""),
            overwrite=TRUE)
}

# if(!is.null(future_variable2.sv.sd.w)){
#   writeRaster(future_variable2.sv.sd.w, file = paste(target_dir, '/Fut_', IPSL, '.sv.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
# 
# if(!is.null(future_variable3.sv.sd.w)){
#   writeRaster(future_variable3.sv.sd.w, file = paste(target_dir, '/Fut_', MIROC, '.sv.sd.w_', sp.n, '.asc', sep=""),overwrite=TRUE)
# }
