...
species_to_run <- c(sp_names_h15, less15_charism)
species_to_run_ord <- sort(species_to_run)
sp_names <- species_to_run_ord[-c(204, 205)]

# sp_names <- unique(sp_names$species)
# sp_names <- sp_names[-c(248, 249)] #"Molossus_coibensis" (ENM files do not exist)
...
Results_diff_area <- matrix(nrow = n, ncol = 19)
...
for(i in 1:n){
  ...
  cu <- raster(paste0("./ENM/outputs/", sp_names[i], "/results/", 
                      "CUR.bin_", sp_names[i], '.asc'))
  cu <- crop(cu, BR)
  cu <- mask(cu, BR)
  
  fu <- raster(paste0("./ENM/outputs/", sp_names[i], "/results/", 
                      '/Fut_bcc.bin_', sp_names[i], '.asc'))
  
  fu <- crop(fu, BR)
  fu <- mask(fu, BR)
  
  cu_cont <- raster(paste0("./ENM/outputs/", sp_names[i], "/results/", 
                           "CUR.cont_", sp_names[i], ".asc"))
  
  cu_cont <- crop(cu_cont, BR)
  cu_cont <- mask(cu_cont, BR)
  
  fu_cont <- raster(paste0("./ENM/outputs/", sp_names[i], "/results/", 
                           '/Fut_bcc.cont_', sp_names[i], '.asc'))
  
  fu_cont <- crop(fu_cont, BR)
  fu_cont <- mask(fu_cont, BR)
  ...
  writeRaster(diff_bin2, filename = paste0("./ENM/Diff_raster/", sp_names[i],
                                           "_diff_bin.tif"),
  ...
  writeRaster(diff_cont, filename = paste0("./ENM/Diff_raster/", sp_names[i],
                                           "_diff_cont.tif"),
              format="GTiff", overwrite=T)
  ...
  Results_diff_area[i, ] <- c(sp_names[i], unchanged_percent_bin, loss_percent_bin,
                              gain_percent_bin, gain_loss_perc_pixel, 
                              unchanged, loss, gain, total_cells,ocorrencia_area_cu, ocorrencia_area_fu,
                              loss_area3, gain_area3, unchaged_area3,
                              perc_loss_rel_pres, perc_gain_rel_pres,
                              net_value_area, net_value_perc,
                              percentage_change)
...
head(Results_diff_area)

write.csv(Results_diff_area, "./ENM/11_area_sp_BR_less15onlycharis.csv", row.names = F)
